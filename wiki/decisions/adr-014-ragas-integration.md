...
否决用 MiniMax（与生产 chat 同源，self-preference bias）、用 Anthropic（中国区网络不稳）。Qwen-Max 与生产 chat 不同源（DashScope 是默认 embedding，不是默认 chat，bias 可接受），中文能力足够，调用稳定，API key 复用 `DASHSCOPE_API_KEY`。embedding 维持 DashScope `text-embedding-v3`，与生产一致避免分布偏移。
