...
> 验收范围：纯 `.md` 文件的入库与问答竖井（验收当时与标准 RAG 并存的全新链路；标准竖井已于其后迁移 031 整体退役）。
...
| 标准竖井影响（验收当期）| 零代码改动（仅在 `AppConfig` 把自动配置的 `vectorStore` 标 primary） |

> 验收后续：标准竖井随迁移 031 整体退役，相关比较项不再适用；md 竖井成为唯一活跃 RAG 链路。
...
- 非 `.md` 格式（当期由标准竖井承载；标准竖井已于迁移 031 退役后，非 md 格式整体不再支持）。
- md 文档参与知识图谱打标 / 文档关系（知识图谱模块随迁移 031 一并删除）。
- 图片 / 流程图视觉理解、OCR、表格 LLM 摘要 child（图文当期交给 [[features/markdown-visual-rag-l2]]，后续被 [[features/markdown-image-rag]] 接续）。
- 历史 `.md`（曾走标准链）自动回填到 md 竖井。
