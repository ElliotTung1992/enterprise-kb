...
| 当前验收阶段 | Phase 0-6 最小闭环 + ADR-010 Trace 解耦 |
| 验收日期 | 2026-05-21 |
| 当前结论 | Trace、离线复现包、评估回放、最小后台页与 Trace 解耦/AOP 生命周期均已实现，并通过模块测试 |

## 验收说明

当前代码已完成 Phase 0-6 最小闭环：数据库表、配置项、模型、Mapper、基础服务、脱敏服务、RAG/Agentic RAG/客服/售后/投诉 Trace 接入、Replay/Eval API、最小后台页，以及 ADR-010 的 Trace 解耦与 AOP turn 生命周期。以下验收场景分为三类：

- **已可验收**：当前 Phase 0 已具备能力。
- **代码已实现，待接口验收**：RAG、客服、售后、投诉、Replay、Eval 与后台页已接入代码，仍需启动本地数据库和浏览器确认运行态结果。
- **架构已验收**：Trace 外层生命周期由 `TraceTurnAspect` 管理，业务入口不再手写 `traceFacade.start(...)`、`trace.complete(...)`、`trace.fail(...)`。
...
| 核心代码位置 | `kb-search/src/main/java/com/enterprise/kb/search/service/impl/CustomerAssistantServiceImpl.java` 的 `chat(...)`、`routedChat(...)`、`legacyChat(...)`、`dispatch(...)` |
| 操作步骤 | 用户发起客服会话 |
...
| 核心代码位置 | `kb-search/src/main/java/com/enterprise/kb/search/service/impl/AfterSalesDomainHandler.java` 的 `buildAgent(...)`、`checkAfterSalesEligibility(...)` |
...
| 核心代码位置 | `kb-search/src/main/java/com/enterprise/kb/search/service/impl/AfterSalesDomainHandler.java` 的 `processHitlInterrupt(...)`；`kb-search/src/main/java/com/enterprise/kb/search/service/impl/CustomerAssistantServiceImpl.java` 的 HITL 兼容路径 |
| 操作步骤 | 查看对应 trace |
...
| 核心代码位置 | `kb-search/src/main/java/com/enterprise/kb/search/service/impl/ComplaintDomainHandler.java` 的 `buildAgent(...)`、`escalateComplaint(...)` |
...
| 核心代码位置 | `kb-search/src/main/java/com/enterprise/kb/search/controller/AgentTraceController.java`；`kb-search/src/main/java/com/enterprise/kb/search/service/impl/AgentTraceServiceImpl.java` |
...
| 核心代码位置 | `kb-search/src/main/java/com/enterprise/kb/search/controller/AgentTraceController.java` 的 `/replay` 接口 |
...
| 核心代码位置 | `AgentTraceController.java` 的 `createEvalCase(...)`；`EvalCaseServiceImpl.java`；`EvalCaseMapper.xml` |
...
| 核心代码位置 | `kb-search/src/main/java/com/enterprise/kb/search/controller/EvalCaseController.java` 的 `/import-jsonl`、`/export-jsonl` 接口 |
...
| 核心代码位置 | `EvalReplayServiceImpl.java`；`EvalRunController.java`；`EvalRunMapper.xml`；`EvalRunResultMapper.xml` |
...
| 核心代码位置 | `kb-app/src/main/resources/static/traces.html`；`kb-app/src/main/resources/static/evals.html`；后台 API：`AgentTraceController`、`EvalCaseController`、`EvalRunController` |
...
Phase 0-6 最小可用代码已完成；ADR-010 的 Trace 解耦与 AOP 外层生命周期已完成，并通过 `kb-search` 与 `kb-app` 模块测试。
...
### 场景 25：客服助手生成 Trace 外壳与路由 Step

| 项目 | 内容 |
|------|------|
| 前置条件 | Trace 开关开启，用户调用客服助手 |
| 核心代码位置 | `kb-search/src/main/java/com/enterprise/kb/search/trace/aop/TraceTurnAspect.java`；`kb-search/src/main/java/com/enterprise/kb/search/service/impl/CustomerAssistantServiceImpl.java` 的 `chat(...)`、`routedChat(...)`、`legacyChat(...)` |
| 操作步骤 | 调用 `POST /api/v1/after-sales/ask` |
| 期望结果 | `agent_traces` 新增 `CUSTOMER_ASSISTANT` |
| 期望结果 | routed 模式记录 `GUARD`、`ROUTER`、最终 `MODEL_CALL` step |
| 当前状态 | 代码已实现，待本地数据库接口验收 |

### 场景 26：售后 HITL 与工具调用可追踪

| 项目 | 内容 |
|------|------|
| 前置条件 | 用户触发售后资格检查或提交售后审核 |
| 核心代码位置 | `kb-search/src/main/java/com/enterprise/kb/search/service/impl/AfterSalesDomainHandler.java` 的 `buildAgent(...)`、`processHitlInterrupt(...)`；`CustomerAssistantServiceImpl` 的 `processHitlInterrupt(...)` |
| 操作步骤 | 客服对话中提供订单号并确认提交售后申请 |
| 期望结果 | Trace step 记录 `checkAfterSalesEligibility` 或 `submitAfterSalesReview` |
| 期望结果 | HITL 中断包含 `tool_call_id`、工具参数，并关联 `REVIEW_REQUEST` |
| 当前状态 | 代码已实现，待本地 HITL 流程验收 |
...
| 前置条件 | 用户提供订单号和投诉描述 |
| 核心代码位置 | `kb-search/src/main/java/com/enterprise/kb/search/service/impl/ComplaintDomainHandler.java` 的 `buildAgent(...)`、`escalateComplaint(...)` |
| 操作步骤 | 触发 `escalateComplaint` 工具 |
| 期望结果 | Trace step 记录 `escalateComplaint` 入参与输出 |
| 期望结果 | Trace step 关联 `COMPLAINT` 业务对象 |
| 当前状态 | 代码已实现，待本地投诉链路验收 |

### 场景 28：管理员查询 Trace 并导出 Replay

| 项目 | 内容 |
|------|------|
| 前置条件 | 用户拥有 `ROLE_SYSTEM_ADMIN`，系统中存在 trace |
| 核心代码位置 | `kb-search/src/main/java/com/enterprise/kb/search/controller/AgentTraceController.java`；`AgentTraceServiceImpl.java` |
| 操作步骤 | 调用 `GET /api/v1/admin/traces`、`GET /api/v1/admin/traces/{traceId}`、`GET /api/v1/admin/traces/{traceId}/replay` |
| 期望结果 | 返回 trace 列表、详情 steps、Replay JSON |
| 期望结果 | 非系统管理员无法访问这些 API |
| 当前状态 | 代码已实现，待本地鉴权验收 |

### 场景 29：从 Trace 生成 Eval Case 并 JSONL 导入导出

| 项目 | 内容 |
|------|------|
| 前置条件 | 用户拥有 `ROLE_SYSTEM_ADMIN`，存在完整 trace |
| 核心代码位置 | `AgentTraceController.java` 的 `createEvalCase(...)`；`EvalCaseController.java`；`EvalCaseServiceImpl.java`；`EvalCaseMapper.xml` |
| 操作步骤 | 调用 `POST /api/v1/admin/traces/{traceId}/eval-case`，再调用 JSONL 导出/导入接口 |
| 期望结果 | `eval_cases` 新增草稿 |
| 期望结果 | JSONL 导入按行返回成功数与错误行号，不因单行错误中断全部导入 |
| 当前状态 | 代码已实现，待本地数据库接口验收 |

### 场景 30：运行最小 Eval Replay

| 项目 | 内容 |
|------|------|
| 前置条件 | 数据集中存在 enabled 的 eval case |
| 核心代码位置 | `kb-search/src/main/java/com/enterprise/kb/search/service/impl/EvalReplayServiceImpl.java`；`EvalRunController.java`；`EvalRunMapper.xml`；`EvalRunResultMapper.xml` |
| 操作步骤 | 调用 `POST /api/v1/admin/eval-runs`，请求体 `{"dataset":"smoke"}` |
| 期望结果 | 新增 `eval_runs` 记录 |
| 期望结果 | 每个 case 新增 `eval_run_results`，包含 PASSED/FAILED 和失败原因 |
| 当前状态 | 代码已实现，待本地数据库接口验收 |

### 场景 31：最小后台页面可查看 Trace 和 Eval

| 项目 | 内容 |
|------|------|
| 前置条件 | 用户已登录并拥有后台 API 权限 |
| 核心代码位置 | `kb-app/src/main/resources/static/traces.html`；`kb-app/src/main/resources/static/evals.html` |
| 操作步骤 | 打开 `/traces.html` 和 `/evals.html` |
| 期望结果 | Trace 页面能查询列表、查看 steps、查看 replay、生成 eval case |
| 期望结果 | Eval 页面能查看/编辑 eval case，并触发 eval run |
| 当前状态 | 代码已实现，待浏览器页面验收 |
...
| 测试项 | 命令/方式 | 结果 |
|--------|-----------|------|
| Search 模块测试 | `JAVA_HOME=/Library/Java/JavaVirtualMachines/jdk-21.jdk/Contents/Home PATH=/Library/Java/JavaVirtualMachines/jdk-21.jdk/Contents/Home/bin:$PATH mvn test -pl kb-search -am` | 通过：105 tests，0 failures，0 errors，1 skipped |
| App 聚合测试 | `JAVA_HOME=/Library/Java/JavaVirtualMachines/jdk-21.jdk/Contents/Home PATH=/Library/Java/JavaVirtualMachines/jdk-21.jdk/Contents/Home/bin:$PATH mvn test -pl kb-app -am` | 通过：Reactor 全部 SUCCESS |
